...
import { useCap } from "../../composables/useCap.js";
...
async function authenticate() {
    ...
    if (cap.enabled.value && !cap.token.value) {
    ...
    try {
        const result = await api.auth.login(password.value, cap.token.value || undefined);
        if (result.success) {
